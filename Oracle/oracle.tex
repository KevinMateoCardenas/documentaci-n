...
\begin{itemize}
  \item \textbf{DBMS\_JOB}: Este Scheduler permite programar trabajos en forma de bloques PL/SQL. Los trabajos se pueden ejecutar en un momento específico o de manera recurrente según una programación establecida.
  
  \item \textbf{DBMS\_SCHEDULER}: Este Scheduler ofrece una funcionalidad más avanzada y flexible en comparación con DBMS\_JOB. Permite programar trabajos utilizando diferentes tipos de acciones, como bloques PL/SQL, procedimientos almacenados, scripts externos, entre otros. Además, brinda opciones de configuración detalladas para controlar la programación, repetición y parámetros de los trabajos.
\end{itemize}

En nuestro ejemplo anterior, utilizamos el Scheduler DBMS\_SCHEDULER para programar un trabajo diario que se ejecute a las 8 AM. A continuación se muestra el código utilizado:
...
En este ejemplo, creamos un trabajo llamado \lstinline{my\_job} que ejecuta un bloque PL/SQL utilizando el procedimiento \lstinline{my\_procedure}. El trabajo está programado para ejecutarse diariamente a las 8 AM.
...
\subsubsection*{Ejemplo de uso de Schedulers en Oracle}

A continuación, se presenta el código necesario para crear las tablas \enquote*{Clientes} y \enquote*{Productos} en Oracle:

\begin{lstlisting}
CREATE TABLE "Clientes" (
    "NumeroIdentificacion" NVARCHAR2(450) NOT NULL,
    "TipoIdentificacion" NVARCHAR2(2000) NOT NULL,
    "Nombre" NVARCHAR2(2000) NOT NULL,
    "Apellido" NVARCHAR2(2000) NOT NULL,
    "FechaCreacion" NVARCHAR2(2000) NOT NULL,
    "FechaModificacion" NVARCHAR2(2000) NOT NULL,
    "CorreoElectronico" NVARCHAR2(2000) NOT NULL,
    "FechaNacimiento" NVARCHAR2(2000) NOT NULL,
    CONSTRAINT "PK_Clientes" PRIMARY KEY ("NumeroIdentificacion")
);
CREATE TABLE "Productos" (
    "NumeroDeCuenta" NVARCHAR2(450) NOT NULL,
    "ClienteId" NVARCHAR2(2000) NOT NULL,
    "Tipo" NVARCHAR2(2000) NOT NULL,
    "estado" NVARCHAR2(2000) NOT NULL,
    "GMF" NVARCHAR2(2000) NOT NULL,
    "saldo" NUMBER(18,2) NOT NULL,
    CONSTRAINT "PK_Productos" PRIMARY KEY ("NumeroDeCuenta")
);
\end{lstlisting}
...
\begin{lstlisting}
create or replace PROCEDURE ELIMINAR_CLIENTES_SIN_PRODUCTOS AS
BEGIN
    DELETE FROM "C##USER"."Clientes"
    WHERE "NumeroIdentificacion" NOT IN (SELECT "ClienteId" FROM "C##USER"."Productos");

    COMMIT;
END;
\end{lstlisting}

Finalmente, utilizaremos el Scheduler \lstinline{DBMS_SCHEDULER} para programar el trabajo que ejecutará el procedimiento cada 30 minutos:

\begin{lstlisting}
BEGIN
DBMS_SCHEDULER.CREATE_JOB (
    job_name        => 'ELIMINAR_CLIENTES',
    job_type        => 'PLSQL_BLOCK',
    job_action      => 'BEGIN ELIMINAR_CLIENTES_SIN_PRODUCTOS; END;',
    start_date      => SYSTIMESTAMP,
    repeat_interval => 'FREQ=MINUTELY; INTERVAL=30',
    end_date        => NULL,
    enabled         => TRUE,
    comments        => 'Eliminar clientes sin productos asociados cada 30 minutos'
);
...
En este ejemplo, creamos un trabajo llamado \lstinline{ELIMINAR_CLIENTES} que ejecuta el procedimiento almacenado \lstinline{ELIMINAR_CLIENTES_SIN_PRODUCTOS}. El trabajo está programado para ejecutarse cada 30 minutos a partir del momento de su creación.
